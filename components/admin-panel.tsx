  ...
  return (
    <section className="min-h-[calc(100svh-68px)] bg-paper px-5 pb-20 pt-[calc(68px+env(safe-area-inset-top)+40px)] text-ink sm:px-8 lg:px-12">
      ...
      {editing && <div className="fixed inset-0 z-[60] overflow-y-auto bg-night/55 px-4 py-6 backdrop-blur-sm sm:px-8 sm:py-10"><form onSubmit={save} className="mx-auto max-w-2xl bg-paper p-5 text-ink sm:p-9">
        <div className="flex items-start justify-between gap-5 border-b border-ink/15 pb-6"><div><p className="eyebrow text-ink/45">{editing.id ? "Редактирование" : "Новый товар"}</p><h2 className="mt-3 font-display text-4xl leading-none">Футболка</h2></div><button type="button" className="icon-button" onClick={() => setEditing(null)} aria-label="Закрыть"><X size={20} /></button></div>
